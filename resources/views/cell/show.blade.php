            ...
            <div class="col-lg-12 margin-tb">
                <div class="col-lg-12 m-0 p-0">
                    <div class="col-lg-2 pull-left pl-0 mb-4">
                        <a class="btn btn-success" href="{{ route('dir.create').'?cell_id=' . $cell->id }}"> Новая папка</a>
                    </div>
                    <div class="col-lg-8">
                        <div class="row col-lg-12">
                            ...
                            <div class="col-lg-8 links pr-0 pt-2 pull-left">
                                ...
                                <div class="col-lg-4 pr-1 pl-0 ml-0 pull-left">
                                    <a href="{{ route('cell.show', ['cell' => $cell->id]) }}?order=id&by={{ $order_by }}&page={{ 1 }}&limit={{ $order_by_field['limit'] }}"><i><?php echo $order_by_field['field'] ? $order_by_field['field'] == 'id' ? '&#'.$arrow.';' : ' ' : ' '; ?></i> Идентификатору</a>
                                </div>
                                <div class="col-lg-3 pl-1">
                                    <a href="{{ route('cell.show', ['cell' => $cell->id]) }}?order=title&by={{ $order_by }}&page={{ 1 }}&limit={{ $order_by_field['limit'] }}"><i><?php echo $order_by_field['field'] ? $order_by_field['field'] == 'title' ? '&#'.$arrow.';' : ' ' : ' '; ?></i> Названию</a>
                                </div>
                                <div class="col-lg-5">
                                    <a href="{{ route('cell.show', ['cell' => $cell->id]) }}?order=created_at&by={{ $order_by }}&page={{ 1 }}&limit={{ $order_by_field['limit'] }}"><i><?php echo $order_by_field['field'] ? $order_by_field['field'] == 'created_at' ? '&#'.$arrow.';' : ' ' : ' '; ?></i> Дате создания</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-1 pull-right ml-3 mb-4 pr-0">
                        <a class="btn btn-primary pull-right" href="{{ route('cell.index') }}"> Назад</a>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="col-md-3">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('dir.destroy',$id) }}" method="POST">
                            <a class="btn btn-info" href="{{ route('dir.show', $id).'?page=1' }}">&#128269;</a>
                            <a class="btn btn-primary" href="{{ route('dir.edit', $id) }}">&#9998;</a>
                            @csrf
                            @method('DELETE')
